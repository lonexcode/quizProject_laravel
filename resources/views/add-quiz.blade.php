        ...
        <div class="bg-white p-8 rounded-2xl shadow-lg w-full max-w-md">
            @if (!Session('quizDetails'))
                ...
                <form action="/add-quiz" method="get" class="space-y-4">
                    ...
                    <button type="submit" class="w-full bg-blue-500 rounded-2xl px-4 py-2 text-white">Add
                        Category</button>
                </form>
            @else
                ...
                <p class="text-green-500 font-bold">
                    Total MCQs:{{ $total }} @if (isset($total) && $total > 0)
                        <a href="/show-quiz/{{ session('quizDetails')->id }}" class="text-yellow-500 text-sm">Show
                            MCQs</a>
                    @endif
                </p>
                ...
                <form action="/add-mcq" method="post">
                    @csrf()
                    <div class="px-4 py-2">
                        <textarea name="question" id="" class="w-full px-4 py-2 border border-gray-300 rounded-2xl focus:outline-none"
                    ...
                    <div class="px-4 py-2">
                        <button type="submit" name="submit"
                            class="w-full bg-blue-500 rounded-2xl px-4 py-2 text-white" value="add-more">Add
                            More</button>
                    </div>
                    <div class="px-4 py-2">
                        <button type="submit" name="submit"
                            class="w-full bg-green-500 rounded-2xl px-4 py-2 text-white" value="done">Add and
                            Submit</button>
                    </div>

                    <div class="px-4 py-2">
                        <a href="/end-quiz"
                            class="w-full bg-red-500 rounded-2xl px-4 py-2 text-white block text-center">End Quiz</a>
                    </div>
                </form>
            @endif
        </div>
